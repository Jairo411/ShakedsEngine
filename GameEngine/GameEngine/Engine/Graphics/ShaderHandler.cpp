...
void ShaderHandler::CreateProgram(const std::string& shaderName_, const std::string& vertexShaderFilePath_, const std::string& fragmentShaderFilePath_) {
	...
	if (vertexShaderCode == "" || fragmentShaderCode == "") {
		// Debug messages are already implemented in ReadShader()
		return;
	}
	...
	if (vertexShader == 0 || fragmentShader == 0) {
		// Debug messages are already implemented in CreateShader()
		return;
	}
...
void ShaderHandler::OnDestroy() {
	...
	programs.clear();
	*/
	// we deleted the program on line 56
}
...
GLuint ShaderHandler::CreateShader(GLenum shaderType_, const std::string& source_, const std::string& shaderName_) {
	...
	if (!compileResult) {
		...
		// shaderLog.begin() returns the iterator that points to the front, where as shaderLog.front() returns the first value... same with end() and back()
